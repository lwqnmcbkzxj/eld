...
router.get('/', async (req, res) => {
    ...
    let session_id = null;
    try {
        session_id = await getActiveSessionID(req_user_id);
    } catch (err) {
        return res.status(500).send(makeResponse(2, err));
    }

    try {
        const db = await mQuery(`select st.session_id, t.trailer_id, t.trailer_external_id, t.trailer_status, t.trailer_created_dt
