...
router.post('/', async (req, res) => {
    ...
    try {
        db = await mQuery(`update session set session_status = 'FINISHED', session_end_dt = current_timestamp 
    ...
    // logout user
    try {
        db = await mQuery(`update user set user_token = '' where user_id = ?`, [req_user_id]);
    } catch (err) {
