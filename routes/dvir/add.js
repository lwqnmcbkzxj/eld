...
const { makeQuery, makeResponse, mQuery, getActiveSessionID, getCurDt } = require('../../utils.js');
...
router.post('/', upload.single('signature'), async function(req, res) { /* vehicle_id, location, has_defects, description, signature, date */
    ...
    const cur_date = (!dt) ? getCurDt() : dt;

    makeQuery(`insert into signature(signature_user_id, signature_src, signature_status, session_id, signature_type, signature_dt) values 
    (?, ?, ?, ?, ?, ?)`, [req_user_id, signature.path, 'ACTIVE', session_id, 'DVIR', cur_date ], (sig_suc) => {
        const signature_id = sig_suc.result.insertId;
        makeQuery(`insert into dvir(vehicle_id, session_id, driver_signature_id, mechanic_signature_id, dvir_location, dvir_deffects_status, dvir_description, creator_user_id, dvir_status, dvir_created_dt) 
        values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [vehicle_id, session_id, signature_id, null, location, defects_status, description, req_user_id, 'ACTIVE', dt], (dvir_suc) => {
